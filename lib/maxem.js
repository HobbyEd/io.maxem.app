 ...
 function deviceInfo(){
	...
	if (maxem_session !=''){
		getUserInfoHelper(maxem_session).then(function(result){ 
			...
			var maxem_boxes = [{ 
				'name': res.maxems[0].Sem_ID , 
				'data': {'id': res.maxems[0].Sem_ID}
			}];
...
function setChargingPoleStatus(email, password, args){
	...
	var maxem_session = Homey.ManagerSettings.get("maxem_session")
	//get the latste device info for this user
	deviceInfo()
	...
	if (authenticated_user !=''){
		...
		var options = {
			uri: `${BASE_URL}maxem/${devices[0].name}/chargepoint/control/enabled`,
			headers:{
				"cookie": maxem_session,
				accept: VERSION_HEADER,
				"content-type": `application/json`
